...
window.warp.WarpWebFactory.memCached(arweave);

async function pageHistory(txId) {
  const { owner, title } = getTxInfo();
...
function create_catch_block(ctx) {
	let t0;
	let t1_value = /*err*/ ctx[8].message + "";
	let t1;
...
// (30:8) {#each hx as page}
function create_each_block(ctx) {
	let tr;
	let td0;
	let t0_value = /*page*/ ctx[5].timestamp + "";
	let t0;
	let t1;
	let td1;
	let t2_value = /*page*/ ctx[5].status + "";
	let t2;
	let t3;
	let td2;
	let a;
	...
	return {
		c() {
			...
			t5 = space();
			attr(a, "class", "link");
			attr(a, "href", a_href_value = "https://arweave.net/" + /*page*/ ctx[5].webpage);
...
// (16:0) <Modal open={historyDialog} on:click={() => (historyDialog = false)}>
function create_default_slot(ctx) {
	...
	handle_promise(pageHistory(/*txId*/ ctx[1]), info);
...
function create_fragment(ctx) {
	...
	modal.$on("click", /*click_handler*/ ctx[3]);
...
function instance($$self, $$props, $$invalidate) {
	...
	return [historyDialog, txId, handleClick, click_handler];
}
